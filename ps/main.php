...
function ps_compute_interest_breakdown($loan) {
    ...
    $total_interest = $regular_interest + $overdue_interest + $penalty_interest + $carried;
    ...
    return [
        ...
        'total_due'           => round(
            $loan->principal + $total_interest + $loan->service_fee,
            2
        ),
...
function ps_get_cash_denominations(): array {
    return [1000, 500, 200, 100, 50, 20, 10, 5, 1];
}

function ps_parse_cash_breakdown($raw): array {
    $decoded = [];
    ...
    foreach (ps_get_cash_denominations() as $denomination) {
        $key = (string) $denomination;
        $qty = isset($decoded[$key]) ? max(0, (int) $decoded[$key]) : 0;
    ...
    return [
        ...
        'raw' => array_combine(
            array_map(static fn($d) => (string) $d, ps_get_cash_denominations()),
            array_map(static fn($row) => $row['quantity'], $rows)
        ),
...
function ps_render_cash_breakdown_panel(array $cash_data, array $metrics, bool $force_render = false): string {
    if (!$force_render && empty($cash_data['has_input'])) {
        return '';
    }

    ob_start();
    ?>
    <div style="margin:10px 0 12px;border:1px solid #000;padding:8px;">
        <div style="font-size:9px;font-weight:700;text-transform:uppercase;margin-bottom:6px;">Cash Count Summary</div>
        <div style="display:grid;grid-template-columns:1.1fr .9fr;gap:10px;align-items:start;">
            <table class="reg" style="margin:0;">
                <thead>
                    <tr>
                        <th>Denomination</th>
                        <th class="r">Qty</th>
                ...
                <tbody>
                    <?php foreach ($cash_data['rows'] as $row): ?>
                    <tr>
                        <td>P <?php echo number_format($row['denomination'], 0); ?></td>
                        <td class="r"><?php echo number_format($row['quantity']); ?></td>
                        <td class="r"><?php echo number_format($row['amount'], 2); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="2" style="text-align:right;">TOTAL COUNTED</td>
                        <td class="r">P <?php echo number_format($cash_data['total_counted'], 2); ?></td>
                    </tr>
                </tfoot>
            </table>
            <table class="reg" style="margin:0;">
                <tbody>
                    <tr><td>Cash Payments In</td><td class="r">P <?php echo number_format((float) ($metrics['cash_in'] ?? 0), 2); ?></td></tr>
                    <tr><td>Cash Loans Out</td><td class="r">P <?php echo number_format((float) ($metrics['cash_out'] ?? 0), 2); ?></td></tr>
                    <tr><td>Non-Cash Payments</td><td class="r">P <?php echo number_format((float) ($metrics['noncash_in'] ?? 0), 2); ?></td></tr>
                    <tr><td style="font-weight:700;">Expected Net Cash</td><td class="r" style="font-weight:700;">P <?php echo number_format((float) ($metrics['expected_cash'] ?? 0), 2); ?></td></tr>
                    <tr><td style="font-weight:700;">Over / Short</td><td class="r" style="font-weight:700;"><?php echo (($metrics['over_short'] ?? 0) >= 0 ? 'P ' : '-P ') . number_format(abs((float) ($metrics['over_short'] ?? 0)), 2); ?></td></tr>
                </tbody>
            </table>
        </div>
    </div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                    ...
                    <div class="bntm-form-group">
                        ...
                        <select name="ticket_tag" id="edit-ticket-tag">
                            <option value="Ranaw">Ranaw</option>
...
if (appraisedEl) appraisedEl.value = appraisedVal.toFixed(2);
    ...
    window.psPrintDocument = function(options) {
        ...
        const pageHeight = activeSettings.height || '210mm';
        const pageOrientation = activeSettings.orientation || 'portrait';
        const marginTop = parseFloat(activeSettings.margin_top || 6);
        const marginRight = parseFloat(activeSettings.margin_right || 8);
        const marginBottom = parseFloat(activeSettings.margin_bottom || 6);
        const marginLeft = parseFloat(activeSettings.margin_left || 8);
        const docZoom = (parseFloat(activeSettings.zoom || 100) || 100) / 100;
        const customCss = activeSettings.custom_css || '';
        let content = '';
        ...
        win.document.write(`<!DOCTYPE html><html><head><title>Pawnshop Document</title><style>
            ...
            @page{size:${pageWidth} ${pageHeight} ${pageOrientation};margin:${marginTop}mm ${marginRight}mm ${marginBottom}mm ${marginLeft}mm;}
            body{zoom:${docZoom};}
            ${customCss}
...
function ps_loans_tab($business_id) {
    ...
    if ($fq) $w .= $wpdb->prepare(" AND (l.ticket_number LIKE %s OR l.root_ticket LIKE %s OR c.first_name LIKE %s OR c.last_name LIKE %s OR EXISTS (
            SELECT 1 FROM {$wpdb->prefix}ps_payments p2
            WHERE p2.loan_id = l.id AND p2.reference_number LIKE %s
        ))", "%{$fq}%", "%{$fq}%", "%{$fq}%", "%{$fq}%", "%{$fq}%");
...
function ps_reports_tab( $business_id ) {
    ...
    ob_start(); ?>
    <form method="GET" id="ps-report-form" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:16px;">
        <input type="hidden" name="tab" value="reports">
        <input type="hidden" name="denomination_data" id="ps-denomination-data" value="<?php echo esc_attr($denomination_data); ?>">
    ...
    .ps-report-subtitle { font-size:13px; color:#64748b; margin-top:4px; }
    .ps-analytics-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:12px; margin-bottom:14px; }
    .ps-analytics-card { border:1px solid #e5e7eb; border-radius:12px; background:#fff; padding:14px; }
    .ps-analytics-label { font-size:11px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:#64748b; }
    .ps-analytics-value { font-size:24px; font-weight:800; color:#0f172a; margin-top:8px; line-height:1.1; }
    .ps-analytics-meta { font-size:12px; color:#64748b; margin-top:6px; }
    .ps-analytics-layout { display:grid; grid-template-columns:minmax(0,1.35fr) minmax(280px,.65fr); gap:14px; }
    .ps-analytics-panel { border:1px solid #e5e7eb; border-radius:12px; background:#fff; padding:16px; }
    .ps-analytics-panel h4 { margin:0 0 12px; font-size:14px; font-weight:800; color:#0f172a; }
    .ps-analytics-table { width:100%; border-collapse:collapse; font-size:13px; }
    .ps-analytics-table th, .ps-analytics-table td { padding:10px 8px; border-bottom:1px solid #eef2f7; text-align:left; }
    .ps-analytics-table th { font-size:11px; text-transform:uppercase; letter-spacing:.08em; color:#64748b; }
    .ps-analytics-table .r { text-align:right; }
    .ps-analytics-empty { padding:22px 12px; text-align:center; color:#94a3b8; font-size:13px; }
    .ps-status-pills { display:flex; flex-wrap:wrap; gap:8px; }
    .ps-status-pill { border:1px solid #dbe2ea; border-radius:999px; padding:7px 10px; font-size:12px; color:#334155; background:#fff; }
    .ps-mini-list { display:grid; gap:10px; }
    .ps-mini-item { border:1px solid #eef2f7; border-radius:10px; padding:10px 12px; background:#fff; }
    .ps-mini-item strong { color:#0f172a; }
    @media (max-width: 1100px) { .ps-analytics-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } .ps-analytics-layout { grid-template-columns:1fr; } }
    @media (max-width: 640px) { .ps-analytics-grid { grid-template-columns:1fr; } }
    </style>

    <div class="ps-report-hero">
        <div>
            <h3 class="ps-report-title"><?php echo esc_html($reports[$rtype]['label'] ?? 'Reports'); ?></h3>
            <div class="ps-report-subtitle">Analytics view on this page. Use <strong>Print PDF</strong> when you need the formal document output.</div>
        </div>
    </div>

    <div id="ps-report-output" class="bntm-form-section" style="padding:16px;background:#f8fafc;border:1px solid #e5e7eb;">
    <?php
    switch ($rtype) {
    ...
    window.psCloseDenominationModal = function() {
        var modal = document.getElementById('ps-denomination-modal');
        if (modal) modal.style.display = 'none';
    };
    window.psUpdateDenominationTotals = function() {
        var total = 0;
        document.querySelectorAll('[data-denom]').forEach(function(input){
            var denom = parseFloat(input.dataset.denom || '0');
            var qty = parseInt(input.value || '0', 10);
            qty = isNaN(qty) || qty < 0 ? 0 : qty;
            total += denom * qty;
            var amountCell = document.getElementById('denom-total-' + input.dataset.denom.replace('.', '-'));
            if (amountCell) amountCell.textContent = (denom * qty).toFixed(2);
            var row = input.closest('[data-denom-row]');
            if (row) row.classList.toggle('is-active', qty > 0);
        });
        var totalEl = document.getElementById('ps-denomination-grand-total');
        if (totalEl) totalEl.textContent = total.toFixed(2);
    };
    window.psClearDenominationInputs = function() {
        document.querySelectorAll('[data-denom]').forEach(function(input){
            input.value = '';
        });
        psUpdateDenominationTotals();
    };
    window.psSaveDenominationAndContinue = function() {
        var payload = {};
        document.querySelectorAll('[data-denom]').forEach(function(input){
            var qty = parseInt(input.value || '0', 10);
            payload[input.dataset.denom] = isNaN(qty) || qty < 0 ? 0 : qty;
        });
        psDenominationValues = payload;
        var serialized = JSON.stringify(payload);
        document.getElementById('ps-denomination-data').value = serialized;
        psReportParams.denominationData = serialized;
        psCloseDenominationModal();
        if (psDenominationModalAction === 'generate') {
            document.getElementById('ps-report-form').submit();
        } else if (psDenominationModalAction === 'print_direct' || psDenominationModalAction === 'print') {
            // For 'print_direct', skip the analytics view and go straight to PDF
            psReportOpenModal(true);
        }
    };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (modal) {
            modal.addEventListener('click', function(e){
                if (e.target === modal) psCloseDenominationModal();
            });
        }
        psUpdateDenominationTotals();
    });
...
function ps_analytics_empty($message = 'No data found for the selected filters.'): string {
    return '<div class="ps-analytics-empty">' . esc_html($message) . '</div>';
}

function ps_analytics_summary($business_id, $from, $to): string {
    global $wpdb;
    $rows = $wpdb->get_results($wpdb->prepare(
        "SELECT l.ticket_number,l.status,l.principal,l.due_date,
    ...
    $status_totals = ['active'=>0,'renewed'=>0,'overdue'=>0,'redeemed'=>0,'forfeited'=>0];
    foreach ($rows as $row) {
        if (isset($status_counts[$row->status])) {
            $status_counts[$row->status]++;
            $status_totals[$row->status] += (float) $row->principal;
        }
    }
    $overdue = array_values(array_filter($rows, static fn($row) => $row->status === 'overdue'));
    $portfolio = array_sum(array_map(static fn($row) => in_array($row->status, ['active','renewed','overdue'], true) ? (float) $row->principal : 0, $rows));
    $total_principal = array_sum(array_column($rows, 'principal'));

    ob_start(); ?>
    <div class="ps-analytics-grid">
        <div class="ps-analytics-card"><div class="ps-analytics-label">Total Tickets</div><div class="ps-analytics-value"><?php echo number_format(count($rows)); ?></div><div class="ps-analytics-meta">Selected date range</div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Principal Released</div><div class="ps-analytics-value">P <?php echo number_format($total_principal, 2); ?></div><div class="ps-analytics-meta">Across all statuses</div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Active Portfolio</div><div class="ps-analytics-value">P <?php echo number_format($portfolio, 2); ?></div><div class="ps-analytics-meta">Active, renewed, overdue</div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Overdue Tickets</div><div class="ps-analytics-value"><?php echo number_format($status_counts['overdue']); ?></div><div class="ps-analytics-meta">Needs follow-up</div></div>
    </div>
    <div class="ps-analytics-layout">
        <div class="ps-analytics-panel">
            <h4>Status Distribution</h4>
            <div class="ps-status-pills">
                <?php foreach ($status_counts as $status => $count): ?>
                <div class="ps-status-pill"><?php echo strtoupper($status); ?>: <strong><?php echo number_format($count); ?></strong> | P <?php echo number_format($status_totals[$status], 2); ?></div>
                <?php endforeach; ?>
            </div>
            <h4 style="margin-top:16px;">Latest Tickets</h4>
            <?php if (empty($rows)) : ?>
                <?php echo ps_analytics_empty(); ?>
            <?php else: ?>
            <table class="ps-analytics-table">
                <thead><tr><th>Ticket</th><th>Customer</th><th>Status</th><th class="r">Principal</th></tr></thead>
                <tbody>
                <?php foreach (array_slice($rows, 0, 8) as $row): ?>
                    <tr><td><?php echo esc_html($row->ticket_number); ?></td><td><?php echo esc_html($row->customer_name); ?></td><td><?php echo ucfirst($row->status); ?></td><td class="r">P <?php echo number_format($row->principal, 2); ?></td></tr>
                <?php endforeach; ?>
                </tbody>
            </table>
            <?php endif; ?>
        ...
        <div class="ps-analytics-panel">
            ...
            <?php if (empty($overdue)) : ?>
                <?php echo ps_analytics_empty('No overdue tickets in this range.'); ?>
            <?php else: ?>
            <div class="ps-mini-list">
                <?php foreach (array_slice($overdue, 0, 6) as $row): ?>
                <div class="ps-mini-item">
                    <strong><?php echo esc_html($row->ticket_number); ?></strong><br>
                    <?php echo esc_html($row->customer_name); ?><br>
                    <span style="color:#64748b;"><?php echo esc_html($row->collateral_desc); ?> | Due <?php echo date('M d, Y', strtotime($row->due_date)); ?></span>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
...
function ps_analytics_loans($business_id, $from, $to): string {
    global $wpdb;
    $rows = $wpdb->get_results($wpdb->prepare(
        "SELECT l.ticket_number,l.principal,l.service_fee,l.loan_date,l.due_date,l.status,
                CONCAT(c.last_name, ', ', c.first_name) AS customer_name,
                col.description AS collateral_desc,col.appraised_value
         FROM {$wpdb->prefix}ps_loans l
         JOIN {$wpdb->prefix}ps_customers c ON c.id=l.customer_id
         JOIN {$wpdb->prefix}ps_collaterals col ON col.id=l.collateral_id
         WHERE l.loan_date BETWEEN %s AND %s
    ...
    $total_principal = array_sum(array_column($rows, 'principal'));
    $total_net = 0;
    $total_appraised = array_sum(array_column($rows, 'appraised_value'));
    foreach ($rows as $row) $total_net += (float)$row->principal - (float)$row->service_fee;
    $avg_loan = count($rows) ? $total_principal / count($rows) : 0;

    ob_start(); ?>
    <div class="ps-analytics-grid">
        <div class="ps-analytics-card"><div class="ps-analytics-label">Loans Granted</div><div class="ps-analytics-value"><?php echo number_format(count($rows)); ?></div><div class="ps-analytics-meta">Within selected dates</div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Principal Released</div><div class="ps-analytics-value">P <?php echo number_format($total_principal,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Net Proceeds</div><div class="ps-analytics-value">P <?php echo number_format($total_net,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Average Loan</div><div class="ps-analytics-value">P <?php echo number_format($avg_loan,2); ?></div></div>
    </div>
    <div class="ps-analytics-layout">
        <div class="ps-analytics-panel">
            <h4>Loan Register</h4>
            ...
            <?php else: ?>
            <table class="ps-analytics-table">
                <thead><tr><th>Ticket</th><th>Customer</th><th>Collateral</th><th class="r">Loan</th><th class="r">Net</th><th>Status</th></tr></thead>
                <tbody>
                <?php foreach (array_slice($rows, 0, 12) as $row): ?>
                    <tr>
                        ...
                        <td><?php echo esc_html($row->collateral_desc); ?></td>
                        <td class="r">P <?php echo number_format($row->principal,2); ?></td>
                        <td class="r">P <?php echo number_format((float)$row->principal - (float)$row->service_fee,2); ?></td>
        ...
        <div class="ps-analytics-panel">
            <h4>Collateral Coverage</h4>
            <div class="ps-mini-list">
                <div class="ps-mini-item"><strong>Total Appraised Value</strong><br><span style="color:#64748b;">P <?php echo number_format($total_appraised,2); ?></span></div>
                <div class="ps-mini-item"><strong>Average Appraised Value</strong><br><span style="color:#64748b;">P <?php echo number_format(count($rows) ? $total_appraised / count($rows) : 0,2); ?></span></div>
                <div class="ps-mini-item"><strong>Principal-to-Appraised Ratio</strong><br><span style="color:#64748b;"><?php echo $total_appraised > 0 ? number_format(($total_principal / $total_appraised) * 100, 2) : '0.00'; ?>%</span></div>
            </div>
        </div>
    </div>
    <?php
...
function ps_analytics_payments($business_id, $from, $to): string {
    global $wpdb;
    $rows = $wpdb->get_results($wpdb->prepare(
        "SELECT p.created_at,p.amount,p.payment_type,p.payment_method,p.interest_amount,p.principal_amount,p.penalty_amount,p.reference_number,
                l.ticket_number, CONCAT(c.last_name, ', ', c.first_name) AS customer_name
    ...
    $principal = array_sum(array_column($rows, 'principal_amount'));
    $penalty = array_sum(array_column($rows, 'penalty_amount'));
    $cash_total = 0; $noncash_total = 0;
    foreach ($rows as $row) {
        if (ps_is_cash_method($row->payment_method)) $cash_total += (float)$row->amount;
        else $noncash_total += (float)$row->amount;
    }

    ob_start(); ?>
    <div class="ps-analytics-grid">
        <div class="ps-analytics-card"><div class="ps-analytics-label">Collections</div><div class="ps-analytics-value">P <?php echo number_format($total,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Interest</div><div class="ps-analytics-value">P <?php echo number_format($interest,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Principal Recovery</div><div class="ps-analytics-value">P <?php echo number_format($principal,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Penalty Income</div><div class="ps-analytics-value">P <?php echo number_format($penalty,2); ?></div></div>
    </div>
    <div class="ps-analytics-layout">
        <div class="ps-analytics-panel">
            ...
            <?php else: ?>
            <table class="ps-analytics-table">
                <thead><tr><th>Date</th><th>Ticket</th><th>Customer</th><th>Type</th><th>Method</th><th class="r">Amount</th></tr></thead>
                <tbody>
                <?php foreach (array_slice($rows, 0, 12) as $row): ?>
                    <tr><td><?php echo date('M d, Y', strtotime($row->created_at)); ?></td><td><?php echo esc_html($row->ticket_number); ?></td><td><?php echo esc_html($row->customer_name); ?></td><td><?php echo ucwords(str_replace('_',' ',$row->payment_type)); ?></td><td><?php echo ucwords(str_replace('_',' ',$row->payment_method)); ?></td><td class="r">P <?php echo number_format($row->amount,2); ?></td></tr>
                <?php endforeach; ?>
                </tbody>
            </table>
            <?php endif; ?>
        ...
        <div class="ps-analytics-panel">
            <h4>Payment Mix</h4>
            <div class="ps-mini-list">
                <div class="ps-mini-item"><strong>Cash</strong><br><span style="color:#64748b;">P <?php echo number_format($cash_total,2); ?></span></div>
                <div class="ps-mini-item"><strong>Non-Cash</strong><br><span style="color:#64748b;">P <?php echo number_format($noncash_total,2); ?></span></div>
                <div class="ps-mini-item"><strong>Total Transactions</strong><br><span style="color:#64748b;"><?php echo number_format(count($rows)); ?></span></div>
            </div>
        </div>
    </div>
    <?php
...
function ps_analytics_daily_summary($business_id, $report_date, array $cash_breakdown = [], string $tag_filter = 'all'): string {
    ...
    $payment_in = array_sum(array_column($rows_payments, 'amount'));
    $interest = array_sum(array_column($rows_payments, 'interest_amount'));
    $cash_in = 0; $noncash_in = 0;
    foreach ($rows_payments as $row) {
        if (ps_is_cash_method($row->payment_method)) $cash_in += (float)$row->amount;
        else $noncash_in += (float)$row->amount;
    }
    ...
    ob_start(); ?>
    <div class="ps-analytics-grid">
        <div class="ps-analytics-card"><div class="ps-analytics-label">Loans Released</div><div class="ps-analytics-value">P <?php echo number_format($loan_out,2); ?></div><div class="ps-analytics-meta"><?php echo number_format(count($rows_loans)); ?> new loans</div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Payments Received</div><div class="ps-analytics-value">P <?php echo number_format($payment_in,2); ?></div><div class="ps-analytics-meta"><?php echo number_format(count($rows_payments)); ?> payment entries</div></div>
    ...
    <div class="ps-analytics-layout">
        <div class="ps-analytics-panel">
            ...
            <table class="ps-analytics-table">
                ...
                <tbody>
                    <tr><td>Cash Payments</td><td class="r">P <?php echo number_format($cash_in,2); ?></td></tr>
                    <tr><td>Non-Cash Payments</td><td class="r">P <?php echo number_format($noncash_in,2); ?></td></tr>
                    <tr><td>Interest Earned</td><td class="r">P <?php echo number_format($interest,2); ?></td></tr>
                    <tr><td>Counted Cash</td><td class="r">P <?php echo number_format($counted_cash,2); ?></td></tr>
                </tbody>
            </table>
        ...
        <div class="ps-analytics-panel">
            ...
            <?php if (empty($rows_payments) && empty($rows_loans)) : ?>
                <?php echo ps_analytics_empty('No loan or payment activity for this date.'); ?>
            <?php else: ?>
            <div class="ps-mini-list">
                <?php foreach (array_slice($rows_payments, 0, 4) as $row): ?>
                    <div class="ps-mini-item"><strong><?php echo esc_html($row->ticket_number); ?></strong><br><?php echo esc_html($row->customer_name); ?><br><span style="color:#64748b;">Payment: P <?php echo number_format($row->amount,2); ?></span></div>
                <?php endforeach; ?>
                <?php foreach (array_slice($rows_loans, 0, 3) as $row): ?>
                    <div class="ps-mini-item"><strong><?php echo esc_html($row->ticket_number); ?></strong><br><?php echo esc_html($row->customer_name); ?><br><span style="color:#64748b;">Loan out: P <?php echo number_format((float)$row->principal - (float)$row->service_fee,2); ?></span></div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
        </div>
...
function ps_analytics_tickets_by_status($business_id, $from, $to, $filter_status): string {
    global $wpdb;
    $status_clause = ($filter_status !== 'all') ? $wpdb->prepare(" AND l.status=%s", $filter_status) : '';
    $rows = $wpdb->get_results($wpdb->prepare(
        "SELECT l.ticket_number,l.root_ticket,l.status,l.principal,l.loan_date,l.due_date,
    ...
    $overdue_days = array_sum(array_map(static fn($row) => max(0, (int)$row->days_past), $rows));

    ob_start(); ?>
    <div class="ps-analytics-grid">
        <div class="ps-analytics-card"><div class="ps-analytics-label">Tickets</div><div class="ps-analytics-value"><?php echo number_format(count($rows)); ?></div><div class="ps-analytics-meta"><?php echo strtoupper($filter_status === 'all' ? 'all statuses' : $filter_status); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Principal Value</div><div class="ps-analytics-value">P <?php echo number_format($total_principal,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Average Principal</div><div class="ps-analytics-value">P <?php echo number_format($avg_principal,2); ?></div></div>
        <div class="ps-analytics-card"><div class="ps-analytics-label">Total Days Past</div><div class="ps-analytics-value"><?php echo number_format($overdue_days); ?></div><div class="ps-analytics-meta">For matching tickets</div></div>
    </div>
    <div class="ps-analytics-panel">
        <h4>Ticket Monitoring</h4>
        <?php if (empty($rows)) : ?>
            <?php echo ps_analytics_empty(); ?>
        <?php else: ?>
        <table class="ps-analytics-table">
            <thead><tr><th>Ticket</th><th>Customer</th><th>Collateral</th><th>Status</th><th>Due Date</th><th class="r">Principal</th><th class="r">Days Past</th></tr></thead>
            <tbody>
            <?php foreach (array_slice($rows, 0, 14) as $row): ?>
                <tr>
                    <td><?php echo esc_html($row->ticket_number); ?></td>
                    <td><?php echo esc_html($row->customer_name); ?></td>
                    <td><?php echo esc_html($row->collateral_desc); ?></td>
                    ...
                    <td class="r">P <?php echo number_format($row->principal,2); ?></td>
                    <td class="r"><?php echo number_format(max(0, (int)$row->days_past)); ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
function ps_rpt_daily_summary( int $business_id, string $report_date, array $b, array $cash_breakdown = [], string $tag_filter = 'all' ): string {
    ...
    $noncash_in = 0;
    foreach ($payments_today as $payment_row) {
        if (ps_is_cash_method($payment_row->payment_method ?? 'cash')) {
            $cash_in += (float) $payment_row->amount;
        } else {
            $noncash_in += (float) $payment_row->amount;
        }
    }
    $cash_out = 0;
    foreach ($loans_today as $loan_row) {
        $net = (float) $loan_row->principal - (float) $loan_row->service_fee;
        if (ps_is_cash_method($loan_row->payment_method ?? 'cash')) {
            $cash_out += $net;
        }
    }
    $expected_cash = round($cash_in - $cash_out, 2);
    $counted_cash = (float) ($cash_breakdown['total_counted'] ?? 0);
    $over_short = round($counted_cash - $expected_cash, 2);
...
function ps_settings_tab($business_id) {
    ...
    <form id="ps-settings-form">
        ...
        <div class="bntm-form-section"><h4 style="margin:0 0 14px;font-size:14px;font-weight:700;">Print Layout</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
                <div class="bntm-form-group">
                    <label>Page Width</label>
                    <input type="text" name="ps_doc_page_width" value="<?php echo esc_attr(bntm_get_setting('ps_doc_page_width','148mm')); ?>" placeholder="148mm">
                    <div style="font-size:11px;color:#6b7280;margin-top:3px;">Examples: `148mm`, `8.5in`, `210mm`</div>
                </div>
                <div class="bntm-form-group">
                    <label>Page Height</label>
                    <input type="text" name="ps_doc_page_height" value="<?php echo esc_attr(bntm_get_setting('ps_doc_page_height','210mm')); ?>" placeholder="210mm">
                    <div style="font-size:11px;color:#6b7280;margin-top:3px;">Examples: `210mm`, `11in`, `13in`</div>
                </div>
                <div class="bntm-form-group">
                    <label>Orientation</label>
                    <select name="ps_doc_orientation">
                        <option value="portrait" <?php selected(bntm_get_setting('ps_doc_orientation','portrait'),'portrait'); ?>>Portrait</option>
                        <option value="landscape" <?php selected(bntm_get_setting('ps_doc_orientation','portrait'),'landscape'); ?>>Landscape</option>
                    </select>
                </div>
                <div class="bntm-form-group"><label>Top Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_margin_top" value="<?php echo esc_attr(bntm_get_setting('ps_doc_margin_top','6')); ?>"></div>
                <div class="bntm-form-group"><label>Right Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_margin_right" value="<?php echo esc_attr(bntm_get_setting('ps_doc_margin_right','8')); ?>"></div>
                <div class="bntm-form-group"><label>Bottom Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_margin_bottom" value="<?php echo esc_attr(bntm_get_setting('ps_doc_margin_bottom','6')); ?>"></div>
                <div class="bntm-form-group"><label>Left Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_margin_left" value="<?php echo esc_attr(bntm_get_setting('ps_doc_margin_left','8')); ?>"></div>
                <div class="bntm-form-group">
                    <label>Document Scale (%)</label>
                    <input type="number" step="1" min="50" max="150" name="ps_doc_zoom" value="<?php echo esc_attr(bntm_get_setting('ps_doc_zoom','100')); ?>">
                    <div style="font-size:11px;color:#6b7280;margin-top:3px;">Use this to shrink or enlarge the whole printed output.</div>
                </div>
                <div class="bntm-form-group" style="grid-column:1/-1;">
                    <label>Custom Print CSS</label>
                    <textarea name="ps_doc_custom_css" rows="5" placeholder="@page { }&#10;.ps-doc-title { font-size: 20px; }&#10;.tk-row { margin-bottom: 6px; }"><?php echo esc_textarea(bntm_get_setting('ps_doc_custom_css','')); ?></textarea>
                    <div style="font-size:11px;color:#6b7280;margin-top:3px;">Optional advanced tweaks for fonts, spacing, borders, and other print styling.</div>
                </div>
            </div>
        </div>
        <div class="bntm-form-section"><h4 style="margin:0 0 14px;font-size:14px;font-weight:700;">Per-Document Print Customization</h4>
            <div style="font-size:13px;color:#6b7280;margin-bottom:14px;">Override the global print layout for each printed document individually. Leave fields blank if you want that document to keep using the global layout above.</div>
            <div style="display:grid;grid-template-columns:1fr;gap:14px;">
                <?php foreach ($doc_defs as $doc_key => $doc_label): ?>
                    <div style="border:1px solid #e5e7eb;border-radius:10px;padding:14px;background:#fafafa;">
                        <div style="font-size:13px;font-weight:700;color:#111827;margin-bottom:12px;"><?php echo esc_html($doc_label); ?></div>
                        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
                            <div class="bntm-form-group">
                                <label>Page Width</label>
                                <input type="text" name="ps_doc_<?php echo esc_attr($doc_key); ?>_page_width" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_page_width','')); ?>" placeholder="Use global">
                            </div>
                            <div class="bntm-form-group">
                                <label>Page Height</label>
                                <input type="text" name="ps_doc_<?php echo esc_attr($doc_key); ?>_page_height" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_page_height','')); ?>" placeholder="Use global">
                            </div>
                            <div class="bntm-form-group">
                                <label>Orientation</label>
                                <?php $doc_orientation = bntm_get_setting('ps_doc_' . $doc_key . '_orientation',''); ?>
                                <select name="ps_doc_<?php echo esc_attr($doc_key); ?>_orientation">
                                    <option value="" <?php selected($doc_orientation,''); ?>>Use global</option>
                                    <option value="portrait" <?php selected($doc_orientation,'portrait'); ?>>Portrait</option>
                                    <option value="landscape" <?php selected($doc_orientation,'landscape'); ?>>Landscape</option>
                                </select>
                            </div>
                            <div class="bntm-form-group"><label>Top Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_<?php echo esc_attr($doc_key); ?>_margin_top" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_margin_top','')); ?>" placeholder="Use global"></div>
                            <div class="bntm-form-group"><label>Right Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_<?php echo esc_attr($doc_key); ?>_margin_right" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_margin_right','')); ?>" placeholder="Use global"></div>
                            <div class="bntm-form-group"><label>Bottom Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_<?php echo esc_attr($doc_key); ?>_margin_bottom" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_margin_bottom','')); ?>" placeholder="Use global"></div>
                            <div class="bntm-form-group"><label>Left Margin (mm)</label><input type="number" step="0.1" min="0" name="ps_doc_<?php echo esc_attr($doc_key); ?>_margin_left" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_margin_left','')); ?>" placeholder="Use global"></div>
                            <div class="bntm-form-group"><label>Scale (%)</label><input type="number" step="1" min="50" max="150" name="ps_doc_<?php echo esc_attr($doc_key); ?>_zoom" value="<?php echo esc_attr(bntm_get_setting('ps_doc_' . $doc_key . '_zoom','')); ?>" placeholder="Use global"></div>
                            <div class="bntm-form-group" style="grid-column:1/-1;">
                                <label>Custom CSS</label>
                                <textarea name="ps_doc_<?php echo esc_attr($doc_key); ?>_custom_css" rows="4" placeholder="Use global"><?php echo esc_textarea(bntm_get_setting('ps_doc_' . $doc_key . '_custom_css','')); ?></textarea>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
...
function ps_get_document_definitions(): array {
    return [
        'pawn_ticket'        => 'Pawn Ticket',
        'renewal_notice'     => 'Renewal Notice',
        'redemption_receipt' => 'Redemption Receipt',
...
function ps_get_print_settings(string $doc_type = ''): array {
    $width = trim((string) bntm_get_setting('ps_doc_page_width', '148mm'));
    $height = trim((string) bntm_get_setting('ps_doc_page_height', '210mm'));
    $orientation = bntm_get_setting('ps_doc_orientation', 'portrait');
    $orientation = in_array($orientation, ['portrait', 'landscape'], true) ? $orientation : 'portrait';

    $margin_top = max(0, (float) bntm_get_setting('ps_doc_margin_top', '6'));
    $margin_right = max(0, (float) bntm_get_setting('ps_doc_margin_right', '8'));
    $margin_bottom = max(0, (float) bntm_get_setting('ps_doc_margin_bottom', '6'));
    $margin_left = max(0, (float) bntm_get_setting('ps_doc_margin_left', '8'));
    $zoom = min(150, max(50, (float) bntm_get_setting('ps_doc_zoom', '100')));
    $custom_css = (string) bntm_get_setting('ps_doc_custom_css', '');

    if ($width === '') $width = '148mm';
    if ($height === '') $height = '210mm';

    if ($doc_type !== '') {
        $prefix = 'ps_doc_' . $doc_type . '_';
        $doc_width = trim((string) bntm_get_setting($prefix . 'page_width', ''));
        $doc_height = trim((string) bntm_get_setting($prefix . 'page_height', ''));
        $doc_orientation = bntm_get_setting($prefix . 'orientation', '');
        $doc_margin_top = bntm_get_setting($prefix . 'margin_top', '');
        $doc_margin_right = bntm_get_setting($prefix . 'margin_right', '');
        $doc_margin_bottom = bntm_get_setting($prefix . 'margin_bottom', '');
        $doc_margin_left = bntm_get_setting($prefix . 'margin_left', '');
        $doc_zoom = bntm_get_setting($prefix . 'zoom', '');
        $doc_custom_css = (string) bntm_get_setting($prefix . 'custom_css', '');

        if ($doc_width !== '') $width = $doc_width;
        if ($doc_height !== '') $height = $doc_height;
        if (in_array($doc_orientation, ['portrait', 'landscape'], true)) $orientation = $doc_orientation;
        if ($doc_margin_top !== '') $margin_top = max(0, (float) $doc_margin_top);
        if ($doc_margin_right !== '') $margin_right = max(0, (float) $doc_margin_right);
        if ($doc_margin_bottom !== '') $margin_bottom = max(0, (float) $doc_margin_bottom);
        if ($doc_margin_left !== '') $margin_left = max(0, (float) $doc_margin_left);
        if ($doc_zoom !== '') $zoom = min(150, max(50, (float) $doc_zoom));
        if (trim($doc_custom_css) !== '') $custom_css = $doc_custom_css;
    }
...
function ps_get_all_print_settings(): array {
    $settings = ['global' => ps_get_print_settings()];
    foreach (array_keys(ps_get_document_definitions()) as $doc_key) {
        $settings[$doc_key] = ps_get_print_settings($doc_key);
    }
    return $settings;
}
...
function ps_page_style(): string {
    ...
    $custom_css = trim((string) $print['custom_css']);
    return '<style>
        @page { size: ' . esc_html($print['width']) . ' ' . esc_html($print['height']) . ' ' . esc_html($print['orientation']) . '; margin: ' . $print['margin_top'] . 'mm ' . $print['margin_right'] . 'mm ' . $print['margin_bottom'] . 'mm ' . $print['margin_left'] . 'mm; }
        * { box-sizing: border-box; }
        body { margin:0; padding:0; font-family:"Courier New",Courier,monospace; font-size:10px; color:#000; background:#fff; zoom:' . ($print['zoom'] / 100) . '; }
...
function bntm_ajax_ps_save_settings() {
    ...
    $settings = [
        ...
        'ps_grace_period', 'ps_auto_forfeit_days',
        'ps_ticket_prefix', 'ps_ticket_mode', 'ps_ticket_start',
        'ps_lost_ticket_fee', 'ps_lost_ticket_notice',
        'ps_doc_page_width', 'ps_doc_page_height', 'ps_doc_orientation',
        'ps_doc_margin_top', 'ps_doc_margin_right', 'ps_doc_margin_bottom', 'ps_doc_margin_left',
        'ps_doc_zoom',
    ];
    foreach ($settings as $key) {
        if (isset($_POST[$key])) {
            bntm_set_setting($key, sanitize_text_field($_POST[$key]));
        }
    }
    ...
    foreach (array_keys(ps_get_document_definitions()) as $doc_key) {
        ...
        foreach ($doc_text_fields as $field) {
            if (isset($_POST[$field])) {
                bntm_set_setting($field, sanitize_text_field($_POST[$field]));
            }
        }

        $css_field = $prefix . 'custom_css';
        if (isset($_POST[$css_field])) {
            bntm_set_setting($css_field, sanitize_textarea_field($_POST[$css_field]));
        }
    }
...
function bntm_ajax_ps_add_payment_method() {
    ...
    bntm_set_setting('ps_payment_methods', json_encode($methods));
...
function bntm_ajax_ps_remove_payment_method() {
    ...
    bntm_set_setting('ps_payment_methods', json_encode(array_values($methods)));
...
function bntm_ajax_ps_record_payment() {
    ...
    $r = $wpdb->insert($wpdb->prefix.'ps_payments', [
        ...
        'processed_by'=>$processed_by,'notes'=>$notes,
    ]);
